...
it.skipIf(!enabled)("ignores invalid carry records without poisoning later legitimate personal bests",async()=>{
 actor.id=(await sql`insert into fkh_users(name) values('Synthetic weekly PB') returning id`)[0].id;
 try {
  const session=(await sql`insert into fkh_sessions(user_id,source,type,date,raw_csv_text) values(${actor.id},'csv','range','2026-09-01','synthetic') returning id`)[0].id;
  const club=(await sql`insert into fkh_clubs(user_id,type,normalized_club_key) values(${actor.id},'7i','weekly-pb') returning id`)[0].id;
  ...
  for(let i=0;i<values.length;i++) {
   ...
   if(i===4)expect((await getWeeklyChangeEvidence(actor.id,new Date('2026-09-07T00:00:00Z'))).personalBestCount).toBe(1);
  }
