...
it.skipIf(!enabled)("keeps older owned sponsors available beyond the newest forty display records",async()=>{
 const ids:string[]=[];
 actor.id=(await sql`insert into fkh_users(name) values('Synthetic sponsor owner') returning id`)[0].id;
 try {
  const old=(await sql`insert into fkh_sponsors(name,slug,owner_user_id,created_at) values('Old owned sponsor',${crypto.randomUUID()},${actor.id},'2000-01-01') returning id`)[0].id;
  ids.push(old);
  for(let i=0;i<41;i++) {
   ids.push((await sql`insert into fkh_sponsors(name,slug,created_at) values('New unrelated sponsor',${crypto.randomUUID()},now()) returning id`)[0].id);
  }
