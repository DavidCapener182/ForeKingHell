...
afterAll(closeDb);
it.skipIf(!enabled)("applies private, friends and public round scopes independently of profile access", async () => {
  const db = postgres(url!, { max: 1 }); const users: string[] = [];
  try {
    users.push(...(await db`insert into fkh_users(name) values('Synthetic scope owner'),('Synthetic scope friend') returning id`).map(row => row.id));
    const owner = await ensureSocialProfileForUser(users[0]);
    await ensureSocialProfileForUser(users[1]);
    const [club] = await db`insert into fkh_clubs(user_id,type,normalized_club_key) values(${users[0]},'7 Iron','synthetic-7i') returning id`;
    ...
    const scorecard = Array.from({length:18}, (_,index) => ({holeNumber:index+1,par:4,score:5,yards:350,name:null}));
    for (let index=0;index<3;index++) await db`insert into fkh_sessions(user_id,source,type,date,raw_csv_text,scorecard_json) values(${users[0]},'manual','real_round',now(),'synthetic',${db.json(scorecard)})`;
    ...
    expect((await getProfilePageData(owner.username))?.stats.gapLadder).toEqual([expect.objectContaining({clubId:club.id,carryMedianYd:150})]);
