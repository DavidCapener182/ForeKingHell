...
vi.mock("@/lib/ai/client", () => ({
  generateAiJson: vi.fn(() => {
    throw new Error("No AI in evidence test");
  }),
}));
...
afterAll(closeDb);
it.skipIf(!enabled)(
  "resolves historical own evidence while excluding foreign, deleted and malformed references",
  async () => {
    ...
    try {
      users.push(
        ...(
          await db`insert into fkh_users(name) values('Synthetic recap owner'),('Synthetic recap other') returning id`
        ).map((row) => row.id),
      );
      actor.id = users[0];
      const [own] =
        await db`insert into fkh_feed_items(user_id,item_type,headline,proof_url,created_at) values(${users[0]},'session','Original source','/sessions/source',now()-interval '1 year') returning id`;
      const [foreign] =
        await db`insert into fkh_feed_items(user_id,item_type,headline) values(${users[1]},'session','Foreign source') returning id`;
      for (let i = 0; i < 13; i++)
        await db`insert into fkh_feed_items(user_id,item_type,headline) values(${users[0]},'status_update','Newer source')`;
      ...
      expect(page.evidenceFeedById[own.id]).toMatchObject({
        headline: "Original source",
        proofUrl: "/sessions/source",
      });
