...
it.skipIf(!enabled)("does not calculate a handicap differential from an incomplete shared scorecard",async()=>{
 actor.id=(await sql`insert into fkh_users(name) values('Synthetic shared partial round') returning id`)[0].id;
 try {
  const card=Array.from({length:18},(_,i)=>({holeNumber:i+1,par:4,score:i===0?5:null,putts:null}));
  const session=(await sql`insert into fkh_sessions(user_id,source,type,date,raw_csv_text,scorecard_json) values(${actor.id},'manual','real_round',now(),'synthetic',${sql.json(card)}) returning id`)[0].id;
  ...
  expect((await SharedRoundPage({params:Promise.resolve({token})})).props.round.handicapDifferential).toBe(0);
  const duplicate=complete.map((hole,index)=>({...hole,holeNumber:index===17?1:hole.holeNumber}));
  await sql`update fkh_sessions set scorecard_json=${sql.json(duplicate)} where id=${session}`;
  expect((await SharedRoundPage({params:Promise.resolve({token})})).props.round.handicapDifferential).toBeNull();
 }finally {await sql`delete from fkh_users where id=${actor.id}`;}
});
it.skipIf(!enabled)("rejects expired, revoked and mismatched-owner shared rounds",async()=>{
 const owners=(await sql`insert into fkh_users(name) values('Synthetic share owner'),('Synthetic share foreign') returning id`).map(row=>row.id);
 try {
  const session=(await sql`insert into fkh_sessions(user_id,source,type,date,raw_csv_text) values(${owners[0]},'manual','real_round',now(),'synthetic') returning id`)[0].id;
  const token=crypto.randomUUID();
  const link=(await sql`insert into fkh_share_links(user_id,token_hash,resource_type,resource_id) values(${owners[0]},${hashShareToken(token)},'round',${session}) returning id`)[0].id;
