...
it.skipIf(!enabled)(
  "uses canonical active offer destination and never records unavailable offers",
  async () => {
    const offers: string[] = [];
    actor.id = (
      await sql`insert into fkh_users(name) values('Synthetic partner click') returning id`
    )[0].id;
    try {
      const offer = (
        await sql`insert into fkh_partner_offers(title,offer_url) values('Synthetic offer','https://example.invalid/canonical') returning id`
      )[0].id;
      offers.push(offer);
      const form = new FormData();
      form.set("offerId", offer);
      form.set("offerUrl", "https://example.invalid/forged");
      form.set("source", "x".repeat(100));
      await expect(recordOfferClickAction(form)).rejects.toMatchObject({
        digest: expect.stringContaining("https://example.invalid/canonical"),
      });
      const clicks = await sql`select user_id,source from fkh_offer_clicks where offer_id=${offer}`;
      expect(clicks).toHaveLength(1);
      expect(clicks[0]).toEqual({ user_id: actor.id, source: "x".repeat(80) });
      ...
      if (offers.length) await sql`delete from fkh_partner_offers where id in ${sql(offers)}`;
