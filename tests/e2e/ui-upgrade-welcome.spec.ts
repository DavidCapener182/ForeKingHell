...
test("Welcome resumes saved source state without changing progress by viewing", async ({
  page,
  context,
}, info) => {
  ...
  try {
    ...
    await db`insert into fkh_user_profiles(user_id,username,display_name,primary_launch_monitor) values(${users[0]},${`welcome-${randomUUID().slice(0, 20)}`},'Synthetic welcome player','Rapsodo')`;
    ...
    for (const surface of ["workbench", "companion"])
      for (const [width, height] of [
        ...
        await page.goto(`/surface/${surface}?next=%2Fwelcome%3Fresume%3D1`, {
        ...
        await expect(
          page.getByRole("link", {
            name: "Continue: Import your first measured session",
            exact: true,
          }),
        ).toHaveAttribute("href", "/import");
        await expect(
          page.getByRole("list", { name: "Setup checklist", exact: true }).getByRole("listitem"),
        ).toHaveCount(6);
        await expect(page.locator('[aria-current="step"]')).toContainText(
          "Import your first measured session",
        );
        expect(
          (await db`select onboarding_completed_at from fkh_users where id=${users[0]}`)[0]
            .onboarding_completed_at,
        ).toBeNull();
        expect(await db`select id from fkh_sessions where user_id=${users[0]}`).toHaveLength(0);
        expect(
          await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1),
        ).toBe(true);
        await page.screenshot({ path: info.outputPath(`P85-${surface}-${width}.png`) });
      }
    expect(
