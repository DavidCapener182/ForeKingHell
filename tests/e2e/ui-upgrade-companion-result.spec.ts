...
test("Companion saved result exposes owned evidence and recovery at every width", async ({
  page,
  context,
}, info) => {
  ...
  try {
    ...
    for (const [width, height] of [
      ...
      await expect(page.getByRole("heading", { name: "Session saved", exact: true })).toBeVisible({
        timeout: 60000,
      });
      await page.getByText("3 saved shots · review and correct", { exact: true }).click();
      const first = page
        .getByRole("region", { name: "Saved shot review" })
        .getByRole("article")
        .first();
      await expect(first).toContainText("Carry 150 yd");
      await first.getByRole("button", { name: "Full evidence", exact: true }).click();
      await expect(page.getByRole("dialog")).toContainText("Shot 1");
      await page.getByRole("button", { name: "Close evidence", exact: true }).click();
      await first.getByRole("button", { name: "Exclude from stats", exact: true }).click();
      await expect(page.getByRole("alertdialog")).toBeVisible();
      await page.getByRole("button", { name: "Cancel", exact: true }).click();
      await page.screenshot({
        path: info.outputPath(`P41-${width}.png`),
        fullPage: true,
        animations: "disabled",
      });
      expect(
      ...
      await page.goto("/companion-runtime/import/result");
      await expect(
        page.getByRole("heading", { name: "Import result unavailable", exact: true }),
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: "Open session history", exact: true }),
