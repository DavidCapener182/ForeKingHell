...
test("G07: server-confirmed reads, failure recovery, dates and bounded drawer", async ({
  page,
}, info) => {
  ...
  const bundle = await build({
    entryPoints: ["tests/fixtures/ui-upgrade/notifications.tsx"],
    bundle: true,
    write: false,
  ...
  await page.route("https://notifications.fixture/**", async (route) => {
    if (route.request().url().endsWith("/api/desktop-workbench/notifications")) {
      if (route.request().method() === "POST") {
        ...
        if (failedWrite) {
          failedWrite = false;
          return route.fulfill({ status: 503, json: { error: "Fixture rejected save" } });
        }
        read = true;
      ...
      return route.fulfill({
        json: {
          items: [
            {
              id: "fixture-invite",
              title: "A long invitation title from the coastal golf fixture",
              detail: "Review this invitation without losing its person, date or selected entity.",
              href: "/challenges/fixture-invite",
              tone: "blue",
              unread: !read,
    ...
    return route.fulfill({
      ...
      body: '<!doctype html><html lang="en" data-theme="clubhouse" style="--font-ui-source:Arial;--font-body:Arial"><head><title>Notification fixture</title></head><body><div id="root"></div></body></html>',
    });
  });
  for (const viewport of [
    ...
    await expect(drawer.getByRole("link", { name: /long invitation/ })).toHaveAttribute(
      "href",
      "/challenges/fixture-invite",
    );
    if (!read) {
