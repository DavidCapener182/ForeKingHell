...
test("Companion Rapsodo course uses full review without desktop handoff", async ({
  page,
}, info) => {
  ...
  const bundle = await build({
    entryPoints: ["tests/fixtures/ui-upgrade/rapsodo-companion.tsx"],
    bundle: true,
    write: false,
  ...
  for (const surface of ["workbench", "companion"])
    for (const [width, height] of [
      ...
      await expect(page.getByLabel("Password", { exact: true })).toHaveValue("synthetic-only");
      await page.getByRole("button", { name: "Connect R-Cloud", exact: true }).click();
      await page.getByRole("button", { name: /Synthetic cloud course/ }).click();
      ...
      await page.screenshot({
        path: info.outputPath(`P42-${surface}-${width}.png`),
        animations: "disabled",
      });
      ...
      expect(imported.importInput.practicePlanId).toBe("fixture-plan-123");
    }
