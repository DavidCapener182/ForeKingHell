...
test("P06 full shot scope and evidence across both surfaces", async ({ page }, info) => {
  ...
  for (const surface of ["workbench", "companion"]) {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Shots", exact: true })).toBeVisible({
      timeout: 60000,
    });
    ...
    const filters = page.getByRole("dialog", { name: "Filter shots" });
    ...
    if (await row.count()) {
      ...
      for (const name of ["Flight", "Source", "History"]) {
        await detail.getByRole("tab", { name, exact: true }).click();
        await expect(detail.getByRole("tab", { name, exact: true })).toHaveAttribute(
          "data-state",
          "active",
        );
      }
      ...
      await detail.getByRole("button", { name: "Done", exact: true }).first().click();
      await expect(row).toBeFocused();
    }
    await page
      .getByRole("searchbox", { name: "Search shots", exact: true })
      .fill("no-matching-shot-fixture-98765");
    await page.getByRole("button", { name: "Search / apply", exact: true }).click();
    await expect(
      page.getByRole("heading", { name: "No matching shots", exact: true }),
    ).toBeVisible();
    await expect(page.getByRole("link", { name: "Clear filters", exact: true })).toBeVisible();
    await page.reload();
    await expect(page.getByRole("searchbox", { name: "Search shots", exact: true })).toHaveValue(
      "no-matching-shot-fixture-98765",
    );
