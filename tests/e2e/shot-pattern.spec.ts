  ...
  test("opens a mapped course shot pattern and supports core toggles", async ({ page }) => {
    ...
    if (directPatternUrl) {
      ...
      const patternLink = page
        .locator('a[href^="/courses/"][href$="/shot-pattern"]')
        .filter({ visible: true })
        .first();
    ...
    await expect(
      page.getByText("Target line", { exact: true }).filter({ visible: true }),
    ).toBeVisible();
    ...
    await expect(page.getByText(/Worst miss/i).filter({ visible: true })).toBeVisible();
    await expect(
      page
        .getByText(/Best 90%/)
        .filter({ visible: true })
    ...
    await expect(page.getByText("420 yd playing").filter({ visible: true }).first()).toBeVisible();
    ...
    await expect(page.getByText("225 yd").filter({ visible: true }).first()).toBeVisible();
    await page.getByRole("spinbutton", { name: "Aim offset yards" }).fill("45");
    await expect(page.getByText("45R").filter({ visible: true }).first()).toBeVisible();
    await expect(page.getByText("Aim 45R").filter({ visible: true }).first()).toBeVisible();
    await page.getByRole("spinbutton", { name: "Target distance yards" }).fill("431");
    await expect(page.getByText("Out of range").filter({ visible: true }).first()).toBeVisible();

    await page.getByRole("button", { name: "Carry" }).click();
    await expect(page.getByText("Carry").filter({ visible: true }).first()).toBeVisible();
