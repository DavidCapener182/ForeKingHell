...
test("G10: confirmation, native form semantics, recovery and bounded dialogs", async ({
  page,
}, info) => {
  test.skip(info.project.name !== "chromium", "Six explicit viewport sizes in one browser.");
  test.setTimeout(180_000);
  const bundle = await build({
    entryPoints: ["tests/fixtures/ui-upgrade/forms.tsx"],
    bundle: true,
    write: false,
  ...
  await page.route("https://forms.fixture/**", (route) =>
    route.fulfill({
      contentType: "text/html",
      body: '<!doctype html><html lang="en" data-theme="clubhouse" style="--font-ui-source:Arial;--font-body:Arial"><head><title>Forms fixture</title></head><body><div id="root"></div></body></html>',
    }),
  );
  const errors: string[] = [];
  ...
  for (const viewport of [
    ...
    await expect(page.getByRole("textbox", { name: "Round name", exact: true })).toHaveValue(
      "Fixture round",
    );
    await trigger.click();
    ...
    await expect(page.getByRole("status", { name: "Result", exact: true })).toHaveText(
      "delete: Fixture round",
    );
    ...
    await expect(page.getByRole("status", { name: "Submissions" })).toHaveText("1");
    await page
      .getByRole("textbox", { name: "Round name", exact: true })
      .fill("Retained edited round");
    await expect(page.locator("[data-dirty-form-bar]")).toBeVisible();
    expect(
      (await page.getByRole("textbox", { name: "Notes", exact: true }).boundingBox())!.height,
    ...
    const finalField = await dialog
      .getByRole("textbox", { name: "Hole 18", exact: true })
      .boundingBox();
    ...
    await page.screenshot({
      path: info.outputPath(`G10-${viewport.width}x${viewport.height}.png`),
    });
    await page.keyboard.press("Escape");
    ...
    await expect(page.getByRole("status", { name: "Adapter result" })).toHaveText(
      "Fixture golfer|7-iron|compare",
    );
  }
