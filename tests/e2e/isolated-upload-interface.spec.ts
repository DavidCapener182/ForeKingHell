...
for (const surface of ["workbench", "companion"] as const) {
  test(`fresh account uploads a CSV through the ${surface} interface`, async ({
    page,
    context,
  }, info) => {
    ...
    const trigger = `full_upload_${Date.now()}`;
    try {
      userId = (
      ...
      await sql.unsafe(
        `create function ${trigger}() returns trigger language plpgsql as $$ begin if new.user_id='${userId!}'::uuid then raise exception 'Synthetic full upload rejection'; end if; return new; end $$`,
      );
      await sql.unsafe(
