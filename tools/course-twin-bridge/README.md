...
Run `npm run bridge:build` on each target operating system. It bundles the service and its WebSocket dependency, injects that bundle into the current Node executable, runs an ephemeral-port self-test, and writes a SHA-256 checksum under `dist/course-twin-bridge/<platform>-<architecture>/`.
...
Node's single-executable support is still marked as active development, so releases must remain pinned to the repository's Node major and run the included executable self-test on every target platform.
