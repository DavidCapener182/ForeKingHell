...
export function MobileMetricRow({
  ...
  value: ReactNode;
  detail?: ReactNode;
  tone?: Parameters<typeof MobileStatus>[0]["tone"];
  href?: string;
  className?: string;
}) {
  return (
    <MobileListRow
      ...
      href={href}
      className={className}
      status={tone ? <MobileStatus label={toneLabel(tone)} tone={tone} /> : undefined}
    />
  );
}
...
function toneLabel(tone: NonNullable<Parameters<typeof MobileStatus>[0]["tone"]>) {
