...
export function AchievementBadgeArtwork({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "pointer-events-none relative hidden min-h-40 overflow-hidden rounded-2xl border border-amber-200/70 bg-gradient-to-br from-white via-amber-50/80 to-emerald-50/70 p-5 shadow-sm md:block",
        className,
      )}
      aria-hidden="true"
    >
      <div className="absolute inset-0 opacity-20">
        <Image src={greenComplexArtwork} alt="" fill sizes="360px" className="object-cover" />
      </div>
      <div className="relative grid h-full grid-cols-2 gap-3">
        {["#f59e0b", "#10b981", "#38bdf8", "#111827"].map((color, index) => (
          <div
            key={color}
            className="grid aspect-square place-items-center rounded-2xl border border-white/70 bg-white/75 shadow-sm"
          >
            <svg viewBox="0 0 64 64" className="size-12" aria-hidden="true">
              <path
                d="M32 6 L51 16 V31 C51 45 42 55 32 59 C22 55 13 45 13 31 V16 Z"
                fill={color}
                opacity={index === 3 ? "0.88" : "0.78"}
              />
              <circle cx="32" cy="31" r="12" fill="white" opacity="0.82" />
              <path
                d="M23 34 C29 24 38 24 43 18"
                fill="none"
                stroke={color}
                strokeWidth="4"
                strokeLinecap="round"
