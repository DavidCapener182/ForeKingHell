...
it("requires the provider key and each exact interval price without exposing configuration", () => {
  for (const plan of billingPlans) for (const name of Object.values(plan.priceEnv)) vi.stubEnv(name, "");
  vi.stubEnv("STRIPE_SECRET_KEY", "synthetic-secret");
  expect(Object.values(getCheckoutAvailability()).every(row => !row.monthly && !row.yearly)).toBe(true);
