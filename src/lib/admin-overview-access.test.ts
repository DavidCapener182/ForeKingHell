...
const mocks = vi.hoisted(() => ({ identity: vi.fn(), db: vi.fn(), select: vi.fn(), limit: vi.fn(), execute: vi.fn() }));
vi.mock("server-only", () => ({}));
...
it.each([getAdminOverviewData, getAdminOperationsSnapshot])("denies an account without active admin access before reading operational totals", async (read) => {
  await expect(read()).rejects.toThrow("redirect:/dashboard");
  expect(mocks.select).toHaveBeenCalledTimes(1);
  expect(mocks.execute).not.toHaveBeenCalled();
});
it.each([getAdminOverviewData, getAdminOperationsSnapshot])("does not open the database when authentication fails", async (read) => {
