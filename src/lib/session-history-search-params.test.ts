...
describe("history search", () => {
 it("keeps source/title matching and unrelated URL context", () => {
  const rows = [{...sessions[0],title:"Long Driver session"}, {...sessions[1],title:"Evening round"}];
  const query = buildSessionHistoryQuery("campaign=summer&session=practice-1", {search:"long driver"}, rows);
  ...
  expect(rows.filter(row=>sessionMatchesHistoryFilters(row,filters)).map(row=>row.id)).toEqual(["practice-1"]);
