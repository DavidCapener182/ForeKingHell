...
import { canApplyAppUpdate, dismissInstallNotice, installNoticeDismissed } from "./pwa-notice-policy";
...
describe("PWA notice policy", () => {
  ...
  it("fails closed for unknown queue state, drafts and active entry routes", () => {
    for (const pathname of ["/import", "/import/result", "/rounds/new", "/play/fixture", "/practice/quick-range"]) {
