...
const mocks = vi.hoisted(() => ({ exchange: vi.fn(), getUser: vi.fn(), profile: vi.fn(), clear: vi.fn(), signOut: vi.fn() }));
vi.mock("@/lib/current-user", () => ({ ensureUserProfile: mocks.profile }));
vi.mock("@/lib/supabase/server", () => ({ clearSupabaseAuthCookies: mocks.clear, createSupabaseServerClient: async () => ({ auth: { exchangeCodeForSession: mocks.exchange, getUser: mocks.getUser, signOut: mocks.signOut } }) }));
import { GET } from "./route";
beforeEach(() => vi.resetAllMocks());
it.each(["missing", "expired"])("preserves the local destination after %s callback code", async (mode) => {
...
it("never preserves an external return destination", async () => {
 const response = await GET(new NextRequest("http://localhost/auth/callback?next=https%3A%2F%2Fexample.invalid"));
 const location = new URL(response.headers.get("location")!);
