...
vi.mock("@/lib/social", () => ({
  updateCurrentSocialProfile: mocks.update,
...
it("preserves all legacy profile media and privacy fields without redirecting", async () => {
  const data = form({
    ...
    bio: " Bio ",
    homeCourse: " Home ",
    primaryLaunchMonitor: " Monitor ",
    publicProfile: "on",
    friendProfile: "on",
    feedVisibilityDefault: "friends",
    leaderboardVisibility: "private",
    roundsVisibility: "private",
    pbsVisibility: "public",
    bagVisibility: "friends",
    achievementsVisibility: "private",
    handicapVisibility: "friends",
    practiceVisibility: "public",
    exactShotsVisibility: "private",
    allowCompare: "on",
  });
  await updateSocialProfileAction(data);
  const expected = mocks.update.mock.calls[0][0];
  mocks.redirect.mockClear();
  expect(await updateSocialProfileFormAction({ ok: false, error: "Old error" }, data)).toEqual({
    ok: true,
  });
...
it("does not confirm success before persistence and returns save errors", async () => {
  ...
  let settled = false;
  const result = updateSocialProfileFormAction({ ok: true }, form({})).then((value) => {
    settled = true;
    return value;
  });
...
it.each(["javascript:alert(1)", "x".repeat(700001)])(
  "rejects invalid media before saving (%#)",
  async (avatarUrl) => {
