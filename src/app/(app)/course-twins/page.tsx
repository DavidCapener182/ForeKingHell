...
import { Cuboid, MapPinned } from "lucide-react";
...
export default async function CourseTwinCataloguePage() {
  ...
  const catalogueModule =
    surface === "workbench" ? await import("@/app/course-twins/course-twin-catalogue") : null;
  const CourseTwinCatalogue = catalogueModule?.CourseTwinCatalogue;
  ...
  return (
    <PageShell>
      {surface === "companion" ? (
        <MobileAppShell className="gap-5">
          <div data-course-twin-mobile-catalogue className="grid gap-5">
            <MobileTopBar
              title="Course Twin"
              actions={
                <MobileIconButton href="/courses" label="View all courses" icon={MapPinned} />
              }
            />
        ...
        <div data-course-twin-desktop-catalogue>
          <PageHeader
          ...
          <section className="grid gap-4" aria-labelledby="course-twin-catalogue-title">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Aintree, Bootle and every checked package are opened from here—no hidden URL
                required.
              </p>
            </div>
            ...
            {twins.length > 0 ? (
              ...
              <AppEmptyState
                ...
                description="No generated course package is available for this account yet."
                primaryAction={
                  <Button asChild variant="outline">
