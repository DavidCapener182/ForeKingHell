...
vi.mock("@/db/client", () => ({
  getDb: () => ({ update: () => ({ set: () => ({ where: mocks.save }) }) }),
}));
...
it("keeps failed persistence recoverable without leaking database errors", async () => {
  ...
  expect(await dismissWelcomeStateAction({}, new FormData())).toEqual({
    error: "Your choice could not be saved. Try again.",
  });
  expect(mocks.save).toHaveBeenCalledOnce();
});
it("preserves the successful Today redirect through the error boundary", async () => {
  mocks.save.mockResolvedValue(undefined);
  await expect(dismissWelcomeStateAction({ error: "old" }, new FormData())).rejects.toMatchObject({
