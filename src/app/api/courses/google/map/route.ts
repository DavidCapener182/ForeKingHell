import { googleStaticMapResponse } from "@/lib/google-course-enrichment";
...
export async function GET(request: Request) {
  ...
  const response = await googleStaticMapResponse({
    height: numberParam(requestUrl, "height") ?? 360,
    latitude,
    longitude,
    width: numberParam(requestUrl, "width") ?? 640,
    zoom: numberParam(requestUrl, "zoom") ?? 15,
  });
