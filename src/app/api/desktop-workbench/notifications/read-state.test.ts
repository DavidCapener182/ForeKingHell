...
vi.mock("@/lib/notification-read-state", () => ({markNotificationsRead: mocks.mark, readNotificationState: vi.fn()}));
...
function request(body: unknown, origin = "https://fixture.test") {
  return new Request("https://fixture.test/api/desktop-workbench/notifications", {method:"POST",headers:{origin,"content-type":"application/json"},body:JSON.stringify(body)});
}

describe("notification read writes", () => {
  beforeEach(() => { vi.resetAllMocks(); mocks.user.mockResolvedValue({id:"fixture-owner"}); mocks.mark.mockResolvedValue(["friend-fixture"]); });
  ...
  it("rejects invalid or unbounded writes without a mutation", async () => {
    for (const ids of [null,[5],[""],Array(81).fill("a"),["a".repeat(121)]]) expect((await POST(request({ids}))).status).toBe(400);
    expect(mocks.mark).not.toHaveBeenCalled();
  });
