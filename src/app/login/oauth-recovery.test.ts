...
vi.mock("@/lib/supabase/server", () => ({
  ...
  createSupabaseServerClient: async () => ({ auth: { signInWithOAuth: mocks.oauth } }),
}));
...
it.each(["configuration", "provider", "exchange"])(
  "retains safe destination after %s failure",
  async (mode) => {
    if (mode === "configuration") mocks.configured.mockReturnValue(false);
    const data = new FormData();
    data.set("provider", mode === "provider" ? "unsupported" : "google");
    data.set("next", "/billing?plan=plus");
    let error: unknown;
    try {
      await signInWithOAuthAction(data);
    } catch (caught) {
      error = caught;
    }
    ...
    expect(destination.searchParams.get("next")).toBe("/billing?plan=plus");
...
it("rejects an external destination on provider failure", async () => {
  ...
  expect(mocks.oauth).toHaveBeenCalledWith({
    provider: "google",
    options: { redirectTo: "http://localhost/auth/callback?next=%2Fdashboard" },
  });
});
...
it("returns a user-facing unavailable message without configuration details", async () => {
  ...
  for (const action of [signInWithPasswordAction, sendMagicLinkAction]) {
    expect(await action({ status: "idle", message: null }, new FormData())).toEqual({
      status: "error",
