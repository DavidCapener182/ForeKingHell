...
export async function recordOfferClickAction(formData: FormData) {
  const destination = await recordOfferClick(
    requiredString(formData, "offerId"),
    formString(formData, "source"),
  );
...
export async function partnerFormAction(
  ...
  try {
    ...
    if (url && !safeExternalUrl(url))
      return { ok: false, error: "Enter a complete http or https website address." };
    ...
    if (operation === "offer") {
      ...
      if (!sponsorId || !title)
        return { ok: false, error: "Choose your sponsor and enter an offer title." };
      if (!offerType || !["affiliate", "discount", "prize", "range_credit"].includes(offerType))
        return { ok: false, error: "Choose an available offer type." };
      await createPartnerOffer({
        sponsorId,
        title,
        offerType,
        description: formString(data, "description"),
        targetContext: formString(data, "targetContext"),
        offerUrl: url,
        couponCode: formString(data, "couponCode"),
      });
    ...
    return {
      ok: false,
      error:
        error instanceof Error && error.message === "Sponsor not found."
          ? "That sponsor is unavailable or is not owned by your account."
          : "The partner changes could not be saved. Try again.",
    };
  }
}
