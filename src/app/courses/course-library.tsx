        ...
        <AppEmptyState
          ...
          description={
            filters.has("favourite") && favourites.size === 0
              ? "Favourite a course from the library, then it will appear in this saved view on this device."
  ...
  return (
    <Card className="group overflow-hidden py-0 transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg motion-reduce:transform-none">
      <div className="relative">
        ...
        <div className="pointer-events-none absolute inset-x-3 bottom-3 flex items-end justify-between gap-2">
          <span className="rounded-md bg-black/70 px-2 py-1 text-xs font-medium text-white backdrop-blur">
            {course.holeCount > 0 ? `${course.holeCount} holes` : "Map pending"}
          </span>
          {course.roundCount > 0 ? (
            <span className="rounded-md bg-black/70 px-2 py-1 text-xs font-medium text-white backdrop-blur">
      ...
      <CardContent className="grid gap-4 p-4">
        ...
        <div className="flex flex-wrap gap-2" aria-label="Course readiness">
          <ReadinessState ready={course.strategyReady} icon={Target} readyLabel="Strategy ready" />
          <ReadinessState
...
function CompactStatus({ ready, label }: { ready: boolean; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 whitespace-nowrap text-sm">
      ...
      <span className={ready ? "font-medium text-foreground" : "text-muted-foreground"}>
        {ready ? label : "Not ready"}
      </span>
    </span>
  );
}
...
export function CoursePreview({
  ...
  course: Pick<
    CourseLibraryEntry,
    "id" | "name" | "latitude" | "longitude" | "mapPreviewAvailable"
  >;
  className?: string;
  priority?: boolean;
}) {
